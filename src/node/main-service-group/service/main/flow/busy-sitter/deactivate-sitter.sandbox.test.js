...
describe(__filename, () => {
  ...
  it('should deactivate sitter', async (done) => {
    ...
    // subscribe event
    natsEx.on('step.main.busy-sitter.deactivate-sitter.ok', async ({sitter}) => {
      const {_id, adId, fatherId} = sitter
      try {
    ...
    // emit step
    natsEx.emit('step.main.busy-sitter.check-ad-spend.overspend', {sitter, spend})
  }, 60 * 1000)
})
