...
describe(__filename, () => {
  ...
  it('should find idle sitters', async (done) => {
    ...
    // setup data
    const sitter = {status: 'idle'}
    const {insertedId: sitterId} = await sitterColl.insertOne(sitter)

    // subscribe event
    natsEx.on('step.main.idle-sitter.find-idle-sitters.ok', async ({sitter}) => {
      const {_id} = sitter
      try {
    ...
    // emit step
    natsEx.emit('step.main.idle-sitter.find-idle-sitters')
  })
})
